...
import { fetchAllUsersPerformance, setCurrentLocation } from './adminSlice';
...
const AdminDashboard = () => {
  ...
  const { locationsWithUsers } = useSelector((state) => state.users);
  ...
  useEffect(() => {
    if (locationsWithUsers?.length > 0 && !selectedLocation) {
      const firstLocation = locationsWithUsers[0];
      setSelectedLocation(firstLocation);
      dispatch(setCurrentLocation(firstLocation.location_id));
    }
  }, [locationsWithUsers, dispatch, selectedLocation]);

  useEffect(() => {
    if (currentLocationId) {
      dispatch(fetchAllUsersPerformance(currentLocationId));
    }
  }, [currentLocationId, dispatch]);

  const handleLocationChange = (location) => {
    setSelectedLocation(location);
    dispatch(setCurrentLocation(location.location_id));
  };
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600">
                Overview of all users' training performance and progress
              </p>
            </div>
              ...
              {/* {locationsWithUsers?.length > 0 && (
                <div className="relative">
                  <select
                    ...
                    onChange={(e) => {
                      const location = locationsWithUsers.find(
                        loc => loc.location_id === e.target.value
                      );
                      if (location) handleLocationChange(location);
                    }}
                    className="bg-white border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  >
            ...
            {loading ? (
              ...
              <div className="px-6 py-12 text-center">
                ...
                <p className="text-gray-500">
                  {selectedLocation 
                    ? `No active users found in ${selectedLocation.location_name}`
